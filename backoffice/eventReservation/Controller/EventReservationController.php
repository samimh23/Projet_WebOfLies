<?php
    ...
    class EventReservationController {
        ...
        public function getEvent($id,$email)
        {
            $sql = "SELECT * FROM eventreservation WHERE email=:email AND id_event=:id_event";
            $db = database::getConnexiondb();
			try{
		...
		{
			$sql="UPDATE eventreservation SET name=:name, prenom=:prenom WHERE id_client=:id";
			$db = database::getConnexiondb();
			try
			{
				$query = $db->prepare($sql);
